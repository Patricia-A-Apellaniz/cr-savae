    ...
    def forward(self, inputs, targets, imp_mask):
        ...
        for index_type, type in enumerate(self.feat_dists):
            ...
            if dist == 'gaussian':
                ...
                ll = torch.log(alpha / lam) + (alpha - 1) * torch.log(targets[:, index_type] / lam) - (targets[:, index_type] / lam).pow(alpha)
            else:
    ...
    def forward(self, inputs, risk_probs, targets, risk):
        ...
        for i in range(len(self.types)):  # For each risk
            ...
            if self.types[i][0] == 'weibull':
                ...
                class_loss = torch.log(risk_probs[:, i]) * (risk == label)  # Only the patients with this risk have this term
                patients_with_current_risk_or_censored = (risk == label) + (risk == 0)
                patients_with_current_risk = (risk == label)
                #loss_ll += - torch.sum(hazard[patients_with_current_risk]) - torch.sum(surv[patients_with_current_risk_or_censored]) - torch.sum(class_loss)
                loss_ll += - torch.sum(hazard[patients_with_current_risk]) - torch.sum(surv) - torch.sum(class_loss)
                #loss_ll += - torch.sum(hazard[patients_with_current_risk]) - torch.sum(surv)  # All patients have this term: if the other risk's happen, then from this risk point of view, these patients are censored
            else:
                raise NotImplementedError('Unknown time distribution to compute loss')
            check_nan_inf(loss_ll, 'Time loss, risk ' + str(label))
        '''
...
class LogLikelihoodLossWithCensoringCompetingInd(nn.Module):  # Independence assumption, deprecated

    def __init__(self, dist_types):
        super(LogLikelihoodLossWithCensoringCompetingInd, self).__init__()
        self.types = dist_types
    ...
    def forward(self, inputs, targets, risk):
        ...
        for i in range(len(self.types)):  # For each risk
            ...
            if self.types[i][0] == 'weibull':
                ...
                #patients_with_current_risk_or_censored = (risk == label) + (risk == 0)
                patients_with_current_risk = (risk == label)
                #loss_ll += - torch.sum(hazard[patients_with_current_risk]) - torch.sum(surv[patients_with_current_risk_or_censored])
                loss_ll += - torch.sum(hazard[patients_with_current_risk]) - torch.sum(surv)  # All patients have this term: if the other risk's happen, then from this risk point of view, these patients are censored
            else:
                raise NotImplementedError('Unknown time distribution to compute loss')
            check_nan_inf(loss_ll, 'Time loss, risk ' + str(label))
